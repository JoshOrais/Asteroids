...
public class Renderer {
	...
	public void renderBackground(Background bg) {
		...
		for (int i = 0; i < bg.getSize(); ++i) {
			...
			backgroundShader.setUniform1f("offx", ((i + 1) * Background.PARRALAX_FACTOR) * bg.getTransform().x);
			backgroundShader.setUniform1f("offy", ((i + 1) * Background.PARRALAX_FACTOR) * bg.getTransform().y);
